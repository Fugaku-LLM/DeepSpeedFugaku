...
def get_model(model_provider_func):
    ...
    if args.DDP_impl == "torch":
        if args.no_cuda:
            model = [
                torchDDP(model_module, device_ids=[], process_group=mpu.get_data_parallel_group())
                for model_module in model
            ]
            return model
...
def training_log(
    ...
    for key in loss_dict:
        if not skipped_iter:
            total_loss_dict[key] = (
                total_loss_dict.get(key, torch.FloatTensor([0.0])) + loss_dict[key]
            )
        else:
    ...
    if iteration % args.tensorboard_log_interval == 0:
        ...
        if args.log_optimizer_states_to_tensorboard and optimizer is not None:
            ...
            if args.zero_stage > 0:
                ...
                opt_stats = torch.FloatTensor(opt_stats)
                torch.distributed.all_reduce(opt_stats, group=mpu.get_data_parallel_group())
                opt_stats_2 = torch.FloatTensor(opt_stats_2)
...
def train(
    ...
    while iteration < args.train_iters and (
        ...
        if args.exit_duration_in_mins:
            ...
            done_cuda = torch.IntTensor([train_time > args.exit_duration_in_mins])
