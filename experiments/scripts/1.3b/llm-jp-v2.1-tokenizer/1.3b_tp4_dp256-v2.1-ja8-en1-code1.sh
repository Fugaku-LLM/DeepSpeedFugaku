...
ja_wiki_weight=0.009021106539
en_wiki_weight=0.02972940872
ja_cc_weight=0.02081523404
en_pile_weight=0.001561568695
code_stack_weight=0.0125

# training setting
train_token_in_billion=178.9
train_token=$(echo "$train_token_in_billion * 1000 * 1000 * 1000" | bc)
train_token=$(echo "$train_token/1" | bc)

# default megatron-deepspeed confgiraution is 3000 million, but they train model using 300 billion tokens. we use 178.9 billion tokens, so we set 1.789 billion tokens to lr-warmup-tokens.
lr_warmup_tokens_in_billion=1.789
